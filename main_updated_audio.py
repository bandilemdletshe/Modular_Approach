...
UDP_PORT = 5005  # New: UDP port for audio alerts
        ...
        while True:
            ...
            for i in np.arange(0, detections.shape[2]):
                ...
                if confidence > 0.5:
                    ...
                    if inches * 0.0254 > 0.1 and inches * 0.0254 < 4.0 and CLASSES[idx] == "person":
                        current_time = time.time()
                        if current_time - last_alert_time >= ALERT_COOLDOWN:
                            alert_message = "Person Detected"
                            tts_queue.put(alert_message)  # Local TTS
                            if udp_socket:  # Send alert via UDP
                                try:
                                    udp_socket.sendto(alert_message.encode(), (JETSON_IP, UDP_PORT))
                                except Exception as e:
                    ...
                    with open(log_filename, 'a', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        writer.writerow([
                            "FRONT_END_LOADER_1", "CxD_1", "LEFT_CAM", 
