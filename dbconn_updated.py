...
def push_to_database(Machine_id, CxD_id, Sensor_id, Class, Confidence, Distance, Technical, Emergency,
                     sensor_position="N/A", sensor_health="N/A", detection_datetime="N/A",
                     accelerometer_timestamp="N/A", accelerometer_xyz="N/A",
                     gyroscope_timestamp="N/A", gyroscope_rads="N/A", image_base64=None):

    connection = None
    try:
        connection = mysql.connector.connect(
            host="srv1924.hstgr.io",
            user="u766595437_afrisam",
            password="Paisa@2025!",
            database="u766595437_afrisam"
        )

        cursor = connection.cursor()

        # Ensure detection_datetime has a valid timestamp
        if detection_datetime == "N/A":
        ...
        # Modified SQL query to include image data
        sql_query = """
        INSERT INTO detections (Machine_id, CxD_id, Sensor_id, Class, Confidence, Distance, Technical, Emergency, 
                                sensor_position, sensor_health, detection_datetime,
                                accelerometer_timestamp, accelerometer_xyz, 
                                gyroscope_timestamp, gyroscope_rads, image_base64)
...
# Function to read from image log CSV file and push new entries
def push_new_entries_to_db(image_log_path="image_log.csv", last_line_read_path="last_line.txt"):
    try:
        ...
        # Check if image log file exists
        if not os.path.exists(image_log_path):
            print("🔹 Image log file does not exist.")
            return

        # Read all lines from image log CSV
        with open(image_log_path, 'r') as csvfile:
            reader = csv.reader(csvfile)
            lines = list(reader)

        print(f"Total lines in image log file: {len(lines)}")  # Debug print
        ...
        temp_file_path = image_log_path + ".tmp"
        processed_count = 0
        ...
        try:
            with open(temp_file_path, 'w', newline='') as temp_file:
                ...
                for i, line in enumerate(new_lines):
                    if len(line) < 12:  # Ensure we have enough columns (including image_base64)
                    ...
                    # Always use current datetime instead of value from CSV
                    detection_datetime = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    image_base64 = line[11] if len(line) > 11 else None

                    # Extract additional data if available
                    accelerometer_timestamp = line[12] if len(line) > 12 and line[12] != "N/A" else "N/A"
                    accelerometer_xyz = line[13] if len(line) > 13 else ""
                    gyroscope_timestamp = line[14] if len(line) > 14 and line[14] != "N/A" else "N/A"
                    gyroscope_rads = line[15] if len(line) > 15 else "N/A"
                    ...
                    if success:
                        processed_count += 1
                        # Don't write this line to the temp file (it's been processed)
                    else:
            ...
            os.replace(temp_file_path, image_log_path)
        ...
        print(f"Error reading or processing image log file: {e}")
